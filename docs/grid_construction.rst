...
In order to construct the mesh in relation to :math:`\delta`, its value needs to be known a priori.
...
There are also no strict rules regarding whether to refine the mesh towards the wall.
...
* Estimate the distribution of :math:`\delta` by running a RANS precursor or using approximate analytical relations, when such are available.
